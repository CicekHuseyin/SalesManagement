...
public class SalesController : Controller
{
    ...
    public IActionResult Create(SalesViewModel model)
    {
        ...
        _stockService.DecreaseStock(model.ProductId, model.Quantity ?? 0);
    ...
    public IActionResult Edit(SalesViewModel model)
    {
        ...
        var sale = _salesService.GetSaleById(model.Id);
        if (sale == null)
            return NotFound();
        ...
        // STOK FARK HESABI
        var quantityDifference = (model.Quantity ?? 0) - (sale.Quantity ?? 0);
